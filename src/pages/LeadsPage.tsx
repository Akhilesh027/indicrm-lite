...
export default function LeadsPage() {
  const { leads, updateLead, convertLeadToCustomer, employees } = useCRMStore();
  ...
  const handleSaveCall = () => {
    ...
    if (callStatus === 'Own Close') {
      convertLeadToCustomer(callPopupLead.id);
    }
          ...
          {callPopupLead && (
            <div className="space-y-4">
              ...
              <div>
                ...
                <div className="grid grid-cols-2 gap-2">
                  {['Own Close', 'Own Loss', 'Call Back', 'No Response', 'Follow Up', 'Demo Completed'].map((status) => (
                    <Button
                      key={status}
                      variant={callStatus === status ? 'default' : 'outline'}
                      size="sm"
                      onClick={() => setCallStatus(status as Lead['status'])}
              ...
              <div className="flex gap-2 pt-4">
                <Button variant="outline" onClick={() => setCallPopupLead(null)} className="flex-1">
                  Cancel
                </Button>
                <Button variant="gradient" onClick={handleSaveCall} className="flex-1">
                  Save Call Log
                </Button>
              </div>
            </div>
